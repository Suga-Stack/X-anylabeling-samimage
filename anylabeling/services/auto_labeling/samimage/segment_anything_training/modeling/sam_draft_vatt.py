...
def thresholding(x, alpha=0.0):
    if alpha == 0:
      return x
    # alpha = 0.01 #tune alpha for vit-l. 0.5 is ok
...
def extract_relation(x, alpha=0.0):
    ...
    mask = torch.eye(similarity.size(1), device=x.device)[None, :, :]  # [1, L, L]
    mask = mask.expand_as(similarity)  # [B, L, L]
    mask = mask.to(torch.float) * float(-1e16)

    masked_similarity = similarity + mask
    output = torch.softmax(masked_similarity, dim=-1)  # [B, L, L]
    ...
    def forward(
        ...
        with ctx():
          ...
          for image_record, curr_embedding in zip(batched_input, image_embeddings):
              ...
              draft_masks = (low_res_masks.detach()).float()
              ...
              _, draft_dense_embeddings = self.prompt_encoder(
                  points=None,
                  boxes=None,
                  masks=draft_masks, 
              )
              # print(draft_masks.shape, low_res_masks.shape, draft_dense_embeddings.shape, dense_embeddings.shape)
              assert draft_dense_embeddings.shape[-1] == dense_embeddings.shape[-1]

              low_draft_masks, _ = self.draft_decoder(
                  image_embeddings=curr_embedding.unsqueeze(0),
                  image_pe=self.prompt_encoder.get_dense_pe(),
                  sparse_prompt_embeddings=sparse_embeddings,
                  dense_prompt_embeddings=draft_dense_embeddings,
                  multimask_output=multimask_output,
              ...
              outputs.append(
                  {
                      ...
                      "low_draft_logits": low_draft_masks,
                      "encoder_embedding": curr_embedding.unsqueeze(0),
                      "image_pe": self.prompt_encoder.get_dense_pe(),
                      "sparse_embeddings":sparse_embeddings,
                      "dense_embeddings":draft_dense_embeddings,
                  }
              )
